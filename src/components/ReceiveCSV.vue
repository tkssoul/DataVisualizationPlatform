...
import type { EChartsType, EChartsCoreOption } from 'echarts/types/dist/echarts'
import type { FormatterDataSet } from '../interfaces/echartsInterfaces'
import type { NpyFile } from '../interfaces/npy'
...
const fetchData = async () => {
  try {
    ...
    const data = response.data as Array<Array<number>>
...
const generateSeriesList = (data: Array<Array<number>>) => {
  // 科技风配色方案数组
  const colorSchemes = [
...
const renderChart = (data: Array<Array<number>>) => {
  // 确保容器存在
  ...
  if (files && files.length > 0 && files[0].type === 'text/csv') {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        npyFiles.value = data.files.map((file: string) => ({
          name: file,
          url: `/download/${file}`,
        }))
      } else {
